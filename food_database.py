        ...
        try:
            self.cursor.execute(
                '''INSERT INTO food_items 
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?)''',
                (food_item.name, food_item.calories, food_item.unit, 
                 food_item.protein, food_item.fat, food_item.carbs, food_item.notion_id)
            )
            self.conn.commit()
            return True
    ...
    def update_food_item(self, food_item):
        ...
        self.cursor.execute(
            '''UPDATE food_items SET 
               ...
               WHERE name=?''',
            (food_item.calories, food_item.unit, food_item.protein, 
             food_item.fat, food_item.carbs, food_item.notion_id, food_item.name)
        )
        self.conn.commit()
